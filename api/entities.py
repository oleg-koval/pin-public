...
class UserProfile(object):
    ...
    def query_following(profile_owner, current_user):
        ...
        """
        query = '''
        select %s, true is_following from users
        join      follows f1 on f1.follow = users.id
        where f1.follower = $id; ''' % (UserProfile.prepare_fields())
        results = db.query(query, vars={'id': profile_owner,
                                        'user_id': current_user}).list()
    ...
    def query_followed_by(profile_owner, current_user):
        ...
        left join follows f2 on f2.follow = users.id and f2.follower = f1.follow
        where f1.follow = $id;''' % (UserProfile.prepare_fields()),
        vars={'id': profile_owner, 'user_id': current_user}).list()
