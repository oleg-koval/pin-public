...
create table admin_users(
    ...
    site_user_id integer references users(id)
);
create table admin_users_roles(
    user_id integer not null references admin_users(id),
    rol_id integer not null references admin_roles(id)
);
create unique index pk_admin_users_roles on admin_users_roles(user_id, rol_id);

create table pin_loader_log (
    pin_loader_id integer not null references admin_users(id),
...
alter table photos add filename text not null default '';
alter table photos add sizes text not null default '';

--- Camilo's change to associate photos to pins
